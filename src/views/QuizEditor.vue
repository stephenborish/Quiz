    ...
    <aside class="w-80 flex flex-col gap-4">
      ...
      <div class="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2">
        <div 
          ...
          :class="[
            activeQuestionIndex === idx ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-slate-200 hover:border-slate-300'
          ]"
        >
          <div class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-[10px] font-black" :class="activeQuestionIndex === idx ? 'text-indigo-600' : 'text-slate-500'">
            {{ idx + 1 }}
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-black text-slate-900 truncate uppercase tracking-widest">{{ q.text || 'Empty Element' }}</p>
      ...
      <GlassCard class="h-full flex flex-col" noPadding>
        <template #header>
           <div class="flex items-center gap-4">
              ...
              <h3 class="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Editing Element {{ Number(activeQuestionIndex) + 1 }}</h3>
           </div>
        </template>
        ...
        <div class="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar">
           ...
           <section class="space-y-6">
              ...
              <textarea 
                ...
                class="w-full bg-slate-50 border border-slate-200 rounded-3xl p-8 text-slate-900 text-xl font-bold focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all min-h-[150px]"
           ...
           <section class="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div class="space-y-6">
                 ...
                 <div 
                   ...
                   class="aspect-video rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500/50 hover:bg-indigo-50 transition-all group overflow-hidden relative"
                 >
                 ...
                 <div class="space-y-4">
                    <div v-for="(opt, idx) in activeQuestion.options" :key="opt.id" class="flex items-center gap-4">
                       <button 
                         ...
                         :class="opt.isCorrect ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-100 text-slate-400 border border-slate-200 hover:border-slate-300'"
                       >
                       ...
                       <div class="flex-1 relative group">
                          <input 
                            v-model="opt.text"
                            type="text" 
                            class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold text-slate-900 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
